...
JSON_VERSION=3.3.0
JSON_DIRECTORY=$DEPENDENCIES_DIR/json-$JSON_VERSION
...
GO_VERSION=1.16.8
GO_DIRECTORY=$DEPENDENCIES_DIR/go-$GO_VERSION
GO_MODS_DIR=$DEPENDENCIES_DIR/go_mods
$SCRIPT_DIR/install-go.sh $DEPENDENCIES_DIR $GO_VERSION $DEPENDENCIES_URL $MAKE_JOBS $GO_MODS_DIR
...
if [[ "$ARCH" == "aarch64" || "$ARCH" == "s390x" ]]; then 
	LUA_VERSION=5.2.4
	LUA_DIRECTORY=$DEPENDENCIES_DIR/lua-$LUA_VERSION
	$SCRIPT_DIR/install-lua.sh $DEPENDENCIES_DIR $LUA_VERSION $DEPENDENCIES_URL $MAKE_JOBS
elif [[ "$ARCH" == "x86_64" ]]; then
	LUA_VERSION=2.0.3
	LUA_DIRECTORY=$DEPENDENCIES_DIR/LuaJIT-$LUA_VERSION
	$SCRIPT_DIR/install-luajit.sh $DEPENDENCIES_DIR $LUA_VERSION $DEPENDENCIES_URL $MAKE_JOBS
else
...
$SCRIPT_DIR/install-yamlcpp.sh $DEPENDENCIES_DIR $YAMLCPP_VERSION $YAMLCPP_DIRECTORY $DEPENDENCIES_URL $MAKE_JOBS $CMAKE_DIRECTORY
...
$SCRIPT_DIR/install-lpeg.sh $DEPENDENCIES_DIR $LPEG_VERSION $DEPENDENCIES_URL $MAKE_JOBS $LUA_DIRECTORY
...
$SCRIPT_DIR/install-lyaml.sh $DEPENDENCIES_DIR $LYAML_VERSION $DEPENDENCIES_URL $MAKE_JOBS $LUA_DIRECTORY $LIBYAML_DIRECTORY
...
$SCRIPT_DIR/install-valgrind.sh $DEPENDENCIES_DIR $VALGRIND_VERSION $DEPENDENCIES_URL $MAKE_JOBS

PYTHON38_VERSION=3.8.3
$SCRIPT_DIR/install-python.sh $DEPENDENCIES_DIR $PYTHON38_VERSION $DEPENDENCIES_URL $MAKE_JOBS $ZLIB_DIRECTORY $OPENSSL_DIRECTORY

PYTHON37_VERSION=3.7.3
$SCRIPT_DIR/install-python.sh $DEPENDENCIES_DIR $PYTHON37_VERSION $DEPENDENCIES_URL $MAKE_JOBS $ZLIB_DIRECTORY $OPENSSL_DIRECTORY

PYTHON36_VERSION=3.6.9
$SCRIPT_DIR/install-python.sh $DEPENDENCIES_DIR $PYTHON36_VERSION $DEPENDENCIES_URL $MAKE_JOBS $ZLIB_DIRECTORY $OPENSSL_DIRECTORY

PYTHON35_VERSION=3.5.9
$SCRIPT_DIR/install-python.sh $DEPENDENCIES_DIR $PYTHON35_VERSION $DEPENDENCIES_URL $MAKE_JOBS $ZLIB_DIRECTORY $OPENSSL_DIRECTORY

PYTHON27_VERSION=2.7.16
PYTHON27_DIRECTORY=$DEPENDENCIES_DIR/Python-$PYTHON27_VERSION
$SCRIPT_DIR/install-python.sh $DEPENDENCIES_DIR $PYTHON27_VERSION $DEPENDENCIES_URL $MAKE_JOBS $ZLIB_DIRECTORY $OPENSSL_DIRECTORY

STATSITE_VERSION=0.7.0-sysdig7
$SCRIPT_DIR/install-statsite.sh $DEPENDENCIES_DIR $STATSITE_VERSION $DEPENDENCIES_URL $MAKE_JOBS $PYTHON27_DIRECTORY $SCONS_DIRECTORY

GPERFTOOLS_VERSION=2.5
$SCRIPT_DIR/install-gperftools.sh $DEPENDENCIES_DIR $GPERFTOOLS_VERSION $DEPENDENCIES_URL $MAKE_JOBS

SONARCLOUD_VERSION=4.3.0.2102
$SCRIPT_DIR/install-sonarcloud.sh $DEPENDENCIES_DIR $SONARCLOUD_VERSION $DEPENDENCIES_URL $MAKE_JOBS

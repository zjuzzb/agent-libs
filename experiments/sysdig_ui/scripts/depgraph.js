function DrDepGrap(parent_div_id, shownow) {
  ...
  This.normalize_node_sizes = function() {
    var j;

    var nodes = This.force.nodes();

    //
    // First pass: find the max and min
    //
    var max = 0;
    var min = Number.MAX_VALUE;
    for(j = 0; j < nodes.length; j++) {
      max = Math.max(max, nodes[j]["c"+This.numcol]);
      min = Math.min(min, nodes[j]["c"+This.numcol]);
    }

    //
    // Second pass: bring everything in the range 5-30px
    //
    for(j = 0; j < nodes.length; j++) {
      var val = nodes[j]["c"+This.numcol];
      val = val * 30 / max;
      if(val < 5) {
        val = 5;
      }

      nodes[j]["cori"+This.numcol] = nodes[j]["c"+This.numcol];
      nodes[j]["c"+This.numcol] = val;
    }
  }
  ...
  This.create_svg = function(data) {
    ...
    //
    // Normalize the node sizes so that we don't have huge or tiny circles
    //    
    This.normalize_node_sizes();
    ...
    This.circle = svg.append("svg:g").selectAll("circle")
      ...
      .enter().append("svg:circle")
        ...
        .on("dblclick", function(d) {
          ...
          g_ChartManager.push(d.clk_targ[0], d.clk_targ[1]);
        })

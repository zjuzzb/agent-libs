...
TEST_F(sys_call_test, DISABLED_forking_execve)
{
	...
	{
		sinsp_evt* e = param.m_evt;
		if(e->get_type() == PPME_SYSCALL_EXECVE_19_E || e->get_type() == PPME_SYSCALL_EXECVE_18_E || e->get_type() == PPME_SYSCALL_EXECVE_17_E)
		{
			//
			// The child should exist
			//
		...
		else if(e->get_type() == PPME_SYSCALL_EXECVE_19_X || e->get_type() == PPME_SYSCALL_EXECVE_18_X || e->get_type() == PPME_SYSCALL_EXECVE_17_X)
		{
			if(callnum == 1)
			{
				sinsp_threadinfo* ti = param.m_inspector->get_thread(ctid, false, true);

...
protected:
	...
	void fake_cri_test_timing(
	    ...
	    float docker_delay,
	    const std::function<void(const callback_param& param, std::atomic<bool>& done)>& callback,
	    const std::function<void(const sinsp_container_info& container,
	                             sinsp_threadinfo* tinfo,
	                             std::atomic<bool>& done)>& new_cb,
	    bool async,
...
TEST_F(container_cri, fake_cri_no_server)
{
	...
	captured_event_callback_t callback = [&](const callback_param& param) {
		...
		EXPECT_TRUE((container_info == nullptr ||
    ...
    bool extra_queries)
{
	...
	auto fake_cri_handle = Poco::Process::launch("./resources/fake_cri",
    ...
    bool want_events)
{
	...
	proc test_proc = proc("./test_helper", {"cri_container_echo"});
	unlink(fake_cri_socket.c_str());
	auto fake_cri_handle =
	    Poco::Process::launch("./resources/fake_cri",
	                          {delay_arg, "unix://" + fake_cri_socket, pb_prefix, runtime});
	auto fake_docker_handle = Poco::Process::launch(
	    "/usr/bin/env",
	    {"python", "./resources/fake_docker.py", to_string(docker_delay), fake_docker_socket});
	auto start_time = time(NULL);
...
struct callback_params
{
	...
	sinsp_container_lookup_state state;
};

void expect_callbacks(std::vector<callback_params>& params,
                      const sinsp_container_info& container,
                      std::atomic<bool>& done)
{
	//	fprintf(stderr, "got evt type=%d state=%d\n", container.m_type, container.m_lookup_state);
	EXPECT_FALSE(params.empty());

	auto exp = params[0];
	//	fprintf(stderr, "exp evt type=%d state=%d\n", exp.type, exp.state);
	EXPECT_EQ(exp.type, container.m_type);
	EXPECT_EQ(exp.state, container.m_lookup_state);
...
TEST_F(container_cri, fake_cri_then_docker)
{
	...
	fake_cri_test_timing(
	    ...
	    [&](const sinsp_container_info& container,
	        sinsp_threadinfo* tinfo,
	        std::atomic<bool>& done) { expect_callbacks(exp_callbacks, container, done); },
	    true);

	EXPECT_TRUE(exp_callbacks.empty());
}

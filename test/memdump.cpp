...
protected:
	...
	virtual void SetUp() { SetUpCaptures(true); }
	...
	string make_token(const string& tag) { return memdump_test::agent_dump_token + ":" + tag; }
	...
	void parse_dump_response(std::shared_ptr<serialized_buffer> item,
	                         draiosproto::dump_response& response)
	{
		dragent_protocol::buffer_to_protobuf((uint8_t*)item->buffer.data(),
		                                     (uint32_t)item->buffer.size(),
		                                     &response);

		g_log->debug(
		    "Dump response token=" + response.token() + " chunk_no=" +
		    to_string(response.chunk_no()) + " final=" + (response.final_chunk() ? "yes" : "no") +
		    " size=" + to_string((response.has_content() ? response.content().size() : 0)) +
		    " final_size= " +
		    to_string((response.has_final_size_bytes() ? response.final_size_bytes() : 0)));
	}
	...
	void perform_overlapping_dumps(uint32_t total)
	{
		...
		// imaginary current time 0s, capture "1" starts

		// current time, 5s. Should be captured by "1" only
		open_test_file("0");
		Poco::Thread::sleep(5000);

		// current time 10s. runs from 0s->20s
		// imaginary time, capture "2" starts
		ASSERT_NO_FATAL_FAILURE({ send_dump_request("1", 10000, 10000, true, false, false); });
		active_dumps.insert("1");
		Poco::Thread::sleep(5000);

		// current time 15s
		// Should be captured by both 1 and 2
		open_test_file("1");
		Poco::Thread::sleep(5000);

		// current time 20s. runs from 10s->30s
		// capture "1" finishes
		ASSERT_NO_FATAL_FAILURE({ send_dump_request("2", 10000, 10000, true, false, false); });
		active_dumps.insert("2");
		Poco::Thread::sleep(5000);

		// current time 25s
		// should be captured by "2" only
		open_test_file("2");

		// imaginary current time 30s, capture "2" finishes
...
{
protected:
	virtual void SetUp() { SetUpCaptures(false); }
};

class memdump_max_one_capture_test : public memdump_test
{
protected:
	virtual void SetUp() { SetUpCaptures(true, 1); }
};
...
TEST_F(memdump_test, overlapping_dumps)
{
	...
	ASSERT_NO_FATAL_FAILURE({ read_trace("1", set<string>{"0", "1"}); });
	ASSERT_NO_FATAL_FAILURE({ read_trace("2", set<string>{"1", "2"}); });
}
...
TEST_F(memdump_no_dragent_events_test, verify_no_dragent_events)
{
	...
	sinsp_evt_formatter* formatter =
	    new sinsp_evt_formatter(inspector.get(),
	                            std::string("*%evt.num %evt.outputtime %evt.cpu %proc.name "
	...
	while (1)
	{
		...
		{
			FAIL() << "Got unexpected error from inspector->next(): " << res
			       << ", last error: " << inspector->getlasterr();
			break;
		}
...
TEST_F(memdump_test, delayed_capture_start)
{
	...
	ASSERT_NO_FATAL_FAILURE({ send_dump_request("delayed", 1000, 3000, false, true, true); });
...
TEST_F(memdump_max_one_capture_test, stop_delayed_capture)
{
	...
	ASSERT_NO_FATAL_FAILURE({ send_dump_request("aborted", 1000, 3000, false, true, true); });

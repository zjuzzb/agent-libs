...
bool run_parser(const std::string& value, sinsp_http_parser &parser)
{
	sinsp_protocol_parser::msg_type mtype =
	   parser.should_parse(0 /* unused */,
			       sinsp_partial_transaction::DIR_UNKNOWN /* unused*/,
			       true /*unused*/,
	...
	{
		success = parser.parse_request(const_cast<char *>(value.c_str()), value.length());
	}
	else if(mtype == sinsp_protocol_parser::MSG_RESPONSE)
	{
		success = parser.parse_response(const_cast<char *>(value.c_str()), value.length());
	}
...
TEST (parse_http_test, variety)
{
	...
	// Request
	{
		...
		EXPECT_EQ(sinsp_http_parser::UM_GET, parser.m_method);
		EXPECT_EQ(std::string("/docs/index.html"), parser.m_path);
		EXPECT_EQ(std::string("gwww.nowhere123.com/docs/index.html"), parser.m_url);
		EXPECT_EQ(std::string("Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)"), parser.m_agent);

		// Response result
		EXPECT_FALSE(parser.m_is_valid);
		EXPECT_EQ(nullptr, parser.m_content_type);
		EXPECT_EQ(0, parser.m_status_code);
	}
	...
	// Response
	{
		...
		EXPECT_EQ(sinsp_http_parser::UM_NONE, parser.m_method);
		EXPECT_EQ(nullptr, parser.m_path);
		EXPECT_EQ(nullptr, parser.m_url);
		EXPECT_EQ(nullptr, parser.m_agent);

		// Response result
		EXPECT_TRUE(parser.m_is_valid);
		EXPECT_EQ(std::string("text/html"), parser.m_content_type);
		EXPECT_EQ(200, parser.m_status_code);
	}
	...
	// Request
	{
		...
		EXPECT_EQ(sinsp_http_parser::UM_GET, parser.m_method);
		EXPECT_EQ(nullptr, parser.m_path);
		EXPECT_EQ(std::string("g/index.html"), parser.m_url);
		EXPECT_EQ(nullptr, parser.m_agent);
	...
	// 501 Method Not Implemented Response
	{
		...
		// Response result
		EXPECT_TRUE(parser.m_is_valid);
		EXPECT_EQ(std::string("text/html; charset=iso-8859-1"), parser.m_content_type);
		EXPECT_EQ(501, parser.m_status_code);
	}
	...
	// 404 File Not Found Request
	{
		...
		EXPECT_EQ(sinsp_http_parser::UM_GET, parser.m_method);
		EXPECT_EQ(nullptr, parser.m_path);
		EXPECT_EQ(std::string("g/t.html"), parser.m_url);
		EXPECT_EQ(nullptr, parser.m_agent);
	...
	// 404 File Not Found Response
	{
		...
		EXPECT_TRUE(parser.m_is_valid);
		EXPECT_EQ(std::string("text/html; charset=iso-8859-1"), parser.m_content_type);
		EXPECT_EQ(404, parser.m_status_code);
	}
	...
	// Wrong HTTP Version Number Response
	{
		...
		EXPECT_TRUE(parser.m_is_valid);
		EXPECT_EQ(std::string("text/html; charset=iso-8859-1"), parser.m_content_type);
		EXPECT_EQ(400, parser.m_status_code);
	}
	...
	// Wrong Request-URI Request
	{
		...
		EXPECT_EQ(sinsp_http_parser::UM_GET, parser.m_method);
		EXPECT_EQ(nullptr, parser.m_path);
		EXPECT_EQ(std::string("gtest.html"), parser.m_url);
		EXPECT_EQ(nullptr, parser.m_agent);
	...
	// Keep-Alive Connection Request
	{
		...
		EXPECT_EQ(sinsp_http_parser::UM_GET, parser.m_method);
		EXPECT_EQ(nullptr, parser.m_path);
		EXPECT_EQ(std::string("g/test.html"), parser.m_url);
		EXPECT_EQ(nullptr, parser.m_agent);
	...
	// Accessing a Protected Resource Request
	{
		...
		EXPECT_EQ(sinsp_http_parser::UM_GET, parser.m_method);
		EXPECT_EQ(nullptr, parser.m_path);
		EXPECT_EQ(std::string("g/forbidden/index.html"), parser.m_url);
		EXPECT_EQ(nullptr, parser.m_agent);
	...
	// Accessing a Protected Resource Response
	{
		...
		EXPECT_TRUE(parser.m_is_valid);
		EXPECT_EQ(std::string("text/html; charset=iso-8859-1"), parser.m_content_type);
		EXPECT_EQ(403, parser.m_status_code);
	}
	...
	// HTTP/1.1 Request
	{
		...
		EXPECT_TRUE(parser.m_is_req_valid);
		EXPECT_EQ(sinsp_http_parser::UM_GET, parser.m_method);
		EXPECT_EQ(std::string("/index.html"), parser.m_path);
		EXPECT_EQ(std::string("g127.0.0.1/index.html"), parser.m_url);
		EXPECT_EQ(nullptr, parser.m_agent);
	...
	// GET Request for Directory Request
	{
		...
		EXPECT_TRUE(parser.m_is_req_valid);
		EXPECT_EQ(sinsp_http_parser::UM_GET, parser.m_method);
		EXPECT_EQ(std::string("/testdir"), parser.m_path);
		EXPECT_EQ(std::string("g127.0.0.1/testdir"), parser.m_url);
		EXPECT_EQ(nullptr, parser.m_agent);
	...
	// GET Request for Directory Response
	{
		...
		EXPECT_TRUE(parser.m_is_valid);
		EXPECT_EQ(std::string("text/html; charset=iso-8859-1"), parser.m_content_type);
		EXPECT_EQ(301, parser.m_status_code);
	}
	...
	// Issue a GET Request through a Proxy Server Request
	{
		...
		EXPECT_TRUE(parser.m_is_req_valid);
		EXPECT_EQ(sinsp_http_parser::UM_GET, parser.m_method);
		EXPECT_EQ(std::string("http://www.amazon.com/index.html"), parser.m_path);
		// Bad URL, tracked in SMAGENT-1215
		//EXPECT_EQ(std::string("ghttp://www.amazon.com/index.html"), parser.m_url);
		EXPECT_EQ(std::string("gwww.amazon.comhttp://www.amazon.com/index.html"), parser.m_url);
		EXPECT_EQ(nullptr, parser.m_agent);
	...
	// Issue a GET Request through a Proxy Server Response
	{
		...
		EXPECT_TRUE(parser.m_is_valid);
		EXPECT_EQ(std::string("text/html; charset=iso-8859-1"), parser.m_content_type);
		EXPECT_EQ(302, parser.m_status_code);
	}
	...
	// "HEAD" Request Method Request
	{
		...
		// Request result
		EXPECT_TRUE(parser.m_is_req_valid);
		EXPECT_EQ(sinsp_http_parser::UM_HEAD, parser.m_method);
		EXPECT_EQ(nullptr, parser.m_path);
		EXPECT_EQ(std::string("h/index.html"), parser.m_url);
		EXPECT_EQ(nullptr, parser.m_agent);
	...
	// "OPTIONS" Request Method Request
	{
		...
		// Request result
		EXPECT_TRUE(parser.m_is_req_valid);
		EXPECT_EQ(sinsp_http_parser::UM_OPTIONS, parser.m_method);
		EXPECT_EQ(std::string("http://www.amazon.com/"), parser.m_path);
		// Bad URL, tracked in SMAGENT-1215
		//EXPECT_EQ(std::string("ohttp://www.amazon.com/"), parser.m_url);
		EXPECT_EQ(std::string("owww.amazon.comhttp://www.amazon.com/"), parser.m_url);
		EXPECT_EQ(nullptr, parser.m_agent);
	...
	// "TRACE" Request Method Request
	{
		...
		// Request result
		EXPECT_TRUE(parser.m_is_req_valid);
		EXPECT_EQ(sinsp_http_parser::UM_TRACE, parser.m_method);
		EXPECT_EQ(std::string("http://www.amazon.com/"), parser.m_path);
		// Bad URL, tracked in SMAGENT-1215
		//EXPECT_EQ(std::string("thttp://www.amazon.com/"), parser.m_url);
		EXPECT_EQ(std::string("twww.amazon.comhttp://www.amazon.com/"), parser.m_url);
		EXPECT_EQ(nullptr, parser.m_agent);
	...
	// "TRACE" Request Method Response
	{
		...
		// Response result
		EXPECT_TRUE(parser.m_is_valid);
		EXPECT_EQ(std::string("message/http"), parser.m_content_type);
		EXPECT_EQ(200, parser.m_status_code);
	}
	...
	// "POST" Request Method
	{
		...
		// Request result
		EXPECT_TRUE(parser.m_is_req_valid);
		EXPECT_EQ(sinsp_http_parser::UM_POST, parser.m_method);
		EXPECT_EQ(std::string("/bin/login"), parser.m_path);
		EXPECT_EQ(std::string("p127.0.0.1:8000/bin/login"), parser.m_url);
		EXPECT_EQ(std::string("Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)"), parser.m_agent);
	...
	// "POST" Request Method (upload)
	{
		...
		EXPECT_TRUE(parser.m_is_req_valid);
		EXPECT_EQ(sinsp_http_parser::UM_POST, parser.m_method);
		EXPECT_EQ(std::string("/bin/upload"), parser.m_path);
		EXPECT_EQ(std::string("ptest101/bin/upload"), parser.m_url);
		EXPECT_EQ(std::string("Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)"), parser.m_agent);
	...
	// Content-Type Negotiation Request 1
	{
		...
		EXPECT_TRUE(parser.m_is_req_valid);
		EXPECT_EQ(sinsp_http_parser::UM_GET, parser.m_method);
		EXPECT_EQ(std::string("/logo"), parser.m_path);
		EXPECT_EQ(std::string("gtest101:8080/logo"), parser.m_url);
		EXPECT_EQ(std::string("Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)"), parser.m_agent);
	...
	// Content-Type Negotiation Response 1
	{
		...
		// Response result
		EXPECT_TRUE(parser.m_is_valid);
		EXPECT_EQ(std::string("image/gif"), parser.m_content_type);
		EXPECT_EQ(200, parser.m_status_code);
	}
	...
	// Language Negotiation and "Options MultiView" Request
	{
		...
		EXPECT_EQ(sinsp_http_parser::UM_GET, parser.m_method);
		EXPECT_EQ(std::string("/index.html"), parser.m_path);
		EXPECT_EQ(std::string("gtest101:8080/index.html"), parser.m_url);
		EXPECT_EQ(std::string("Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)"), parser.m_agent);

...
TEST_F(sys_call_test, usrevts_1)
{
	...
	p.process_event_data(buffer, sizeof(doc) - 1);

	EXPECT_EQ(sinsp_usrevtparser::RES_OK, p.m_res);
	EXPECT_EQ(true, p.m_is_enter);
	EXPECT_EQ("12435", string(p.m_id));
...
TEST_F(sys_call_test, usrevts_2)
{
	...
	EXPECT_EQ(sinsp_usrevtparser::RES_OK, p.m_res);
	EXPECT_NE(true, p.m_is_enter);
	EXPECT_EQ("", string(p.m_id));
...
TEST_F(sys_call_test, usrevts_3)
{
	...
	EXPECT_EQ(sinsp_usrevtparser::RES_FAILED, p.m_res);
}
...
TEST_F(sys_call_test, usrevts_29)
{
	...
	EXPECT_EQ(sinsp_usrevtparser::RES_TRUNCATED, p.m_res);
}
...
TEST_F(sys_call_test, usrevts_36)
{
	sinsp_usrevtparser p;

	char doc1[] = "[1243";
	char doc2[] = "5, >, [\"mysql\", \"que";
	char doc3[] = "ry\", \"init\"], [{\"argname1\":\"argval1\"}, {\"argname2\":\"argval2\"}, {\"argname3\":\"argval3\"}]]";
...
TEST_F(sys_call_test, usrevts_37)
{
	sinsp_usrevtparser p;
...
TEST_F(sys_call_test, usrevts_38)
{
	sinsp_usrevtparser p;

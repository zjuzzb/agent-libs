...
TEST(procinfo, process_not_existent)
{
	...
	//
	EXPECT_EQ(NULL, &*inspector.get_thread_ref(0xffff, false, true));
	...
	//
	sinsp_threadinfo* tinfo = &*inspector.get_thread_ref(0xffff, true, true);
	...
	// ...and confirm
	//
	tinfo = &*inspector.get_thread_ref(0xffff, false, true);
	EXPECT_NE((sinsp_threadinfo*)NULL, tinfo);
	if(tinfo)
	{
		EXPECT_EQ("<NA>", tinfo->m_comm);
	...
	captured_event_callback_t callback = [&](const callback_param& param)
	{
		...
		else if(type == PPME_SYSCALL_PRLIMIT_X)
		{
			...
			if(callnum == 1)
			{
				...
				EXPECT_EQ((int64_t)-1, NumberParser::parse64(e->get_param_value_str("newmax", false)));

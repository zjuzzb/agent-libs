...
public:
	...
	void SetUp()
	{
		ASSERT_TRUE(m_logger == nullptr);
		m_logger = new sinsp_logger();
		ASSERT_TRUE(m_logger != nullptr);
...
protected:
	/**
	 * Returns a reference to the test instance of the logger.
	 */
	sinsp_logger& get_logger()
	{
		return *m_logger;
	}
	 ...
	 */
	void count_substrings(const std::string& target,
	 ...
	 * function.
	 */
	static const std::string& get_callback_output()
	{
		return s_cb_output;
	}
...
{
	ASSERT_EQ(get_logger().get_log_output_type(), sinsp_logger::OT_NONE);
	ASSERT_EQ(get_logger().get_severity(), sinsp_logger::SEV_INFO);
}
...
TEST_F(sinsp_logger_test, low_severity_not_logged)
{
	...
	get_logger().set_severity(sinsp_logger::SEV_ERROR);
	ASSERT_EQ(get_logger().get_severity(), sinsp_logger::SEV_ERROR);

	get_logger().add_stdout_log();
	ASSERT_EQ(get_logger().get_log_output_type(), sinsp_logger::OT_STDOUT);
...
TEST_F(sinsp_logger_test, log_standard_error)
{
	...
	get_logger().add_stderr_log();
	ASSERT_EQ(get_logger().get_log_output_type(), sinsp_logger::OT_STDERR);
...
TEST_F(sinsp_logger_test, log_file)
{
	...
	get_logger().add_file_log(filename);
	ASSERT_EQ(get_logger().get_log_output_type(), sinsp_logger::OT_FILE);
...
{
	get_logger().add_callback_log(log_callback_fn);
	ASSERT_EQ(get_logger().get_log_output_type(), sinsp_logger::OT_CALLBACK);

	get_logger().log(DEFAULT_MESSAGE, sinsp_logger::SEV_FATAL);
...
TEST_F(sinsp_logger_test, log_stderr_multithreaded)
{
	...
	{
		threads[i] = std::thread([this, message, NUM_LOGS]()
		{

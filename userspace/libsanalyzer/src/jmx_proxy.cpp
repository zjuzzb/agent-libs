...
void java_bean_attribute::to_protobuf(draiosproto::jmx_attribute* attribute,
      ...
      m_total_metrics(0)
{
	for (const auto& attribute : json["attributes"])
	{
		...
		if (ml)
		{
			...
			// eg. when alias is excluded on the list but name is not on the list at all (thus
			// allowed by default), metric is still allowed by the name absence from the list; so,
			// we have to find out whether the filter permission status is explicit (filter pattern)
			// or implicit (not on the list) and apply folowing logic:
			//
			// 1. if name nor alias is found in the list, metric is included
			// 2. if [alias OR name OR both] are found in the list, metric is included/excluded
			...
			metric_included =
			    ((alias_pos == nopos) && (name_pos == nopos));    // 1. (neither in the list)
...
unsigned int java_bean::to_protobuf(draiosproto::jmx_bean* proto_bean,
                                    unsigned sampling,
                                    unsigned limit,
...
unsigned int java_process::to_protobuf(draiosproto::java_info* protobuf,
                                       unsigned sampling,
                                       unsigned limit,
                                       const std::string& limit_type,
                                       unsigned max_limit) const
{
	...
	for (auto bean_it = m_beans.cbegin(); bean_it != m_beans.cend(); ++bean_it)
	{
		if (protobuf && (limit > emitted_attributes))
		{
			...
			emitted_attributes += bean_it->to_protobuf(protobean,
			                                           sampling,
			                                           limit - emitted_attributes,
			                                           limit_type,
			                                           max_limit);
		}
		else if (metric_limits::log_enabled())
...
Json::Value jmx_proxy::tinfo_to_json(thread_analyzer_info* tinfo)
{
	...
	for (const auto& arg : tinfo->m_args)
	{
		...
		if ((arg.find("-D") == 0 && arg.find("jmx") != std::string::npos &&
		     arg.size() < MAX_ARG_SIZE) ||
		    (arg.find("UsePerfData") != std::string::npos) || (read_jar_arg == true))
		{
	...
	LOG_DEBUG(
	    "Adding process for JMX getMetrics command: "
	    "pid: %" PRIu64 " vpid: %" PRIu64 " root: %s args: %s",
	    tinfo->m_pid,
	    tinfo->m_vpid,
	    tinfo->m_root.c_str(),
	    args.c_str());
...
void jmx_proxy::send_get_metrics(const std::vector<thread_analyzer_info*>& processes)
{
	...
	LOG_DEBUG(
	    "Sending JMX getMetrics command for %lu "
	    "processes, command size %zu bytes",
	    tinfo_count,
	    command_data.size());
...
std::unordered_map<int, java_process> jmx_proxy::read_metrics(const metric_limits::sptr_t& ml)
{
	...
	try
	{
		...
		if (json_data.size() > 0)
		{
			...
			bool parse_ok =
			    m_json_reader.parse(&json_data[0], &json_data[json_data.size()], json_obj, false);

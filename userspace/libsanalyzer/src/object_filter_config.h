...
{
	explicit port_filter_rule()
	    : m_include(false), m_use_set(false), m_range_start(0), m_range_end(0)
	{
	}
...
struct filter_condition
{
	...
	filter_condition() : m_param_type(param_type::none), m_param(""), m_pattern(""), m_port_match()
	{
	}
	...
	filter_condition(param_type type,
	    ...
	    : m_param_type(type), m_param(param), m_pattern(pattern), m_port_match(port_match)
	{
	}
...
struct filter_rule
{
	...
	filter_rule(std::string name,
	    ...
	    : m_name(name), m_include(include), m_cond(cond), m_config(config)
	{
	}

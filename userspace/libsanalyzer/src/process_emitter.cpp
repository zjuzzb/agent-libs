...
void process_emitter::filter_top_programs(Iterator progtable_begin,
                                          ...
                                          std::set<thread_analyzer_info*>& processes_to_emit)
{
	...
	std::vector<thread_analyzer_info*> prog_sortable_list;
	...
	for (auto ptit = progtable_begin; ptit != progtable_end; ++ptit)
	{
		...
		if (m_simpledriver_enabled &&
		    (!cs_only || (*ptit)->m_procinfo->m_proc_metrics.m_net.m_count != 0))
		{
	...
	// did it.
	for (uint32_t i = 0; i < how_many && prog_sortable_list[i]->m_cpuload > 0; i++)
	{
		processes_to_emit.insert(prog_sortable_list[i]);
	}
	...
	if (!m_nodriver)
	{
		...
		for (uint32_t i = 0;
		     i < how_many &&
		     prog_sortable_list[i]->m_procinfo->m_proc_metrics.m_io_net.get_tot_bytes() > 0;
		     i++)
		{
			processes_to_emit.insert(prog_sortable_list[i]);
		}
...
void process_emitter::filter_process(thread_analyzer_info* tinfo,
                                     ...
                                     std::set<thread_analyzer_info*>& blacklist_processes)
{
	...
	if (m_process_manager.get_flush_filter().matches(
	        tinfo,
	        tinfo,
	        container_info,
	        nullptr,  // infra state
...
void process_emitter::emit_processes(
    ...
    std::set<thread_analyzer_info*>& emitted_processes)
{
	...
	for (const auto& container_it : progtable_by_container)
	{
		const auto container_info =
		    m_inspector.m_container_manager.get_container(container_it.first);
	...
	if (!m_inspector.is_capture() &&
	    processes_to_emit.size() < process_manager::c_process_limit.get_value())
	{
		// Filter top active programs
		filter_top_programs(
		    progtable.begin(),
    ...
    bool high_priority)
{
	...
	if (tot.m_count != 0)
	{
		...
		procinfo.m_proc_transaction_metrics.to_protobuf(proc->mutable_transaction_counters(),
		                                                ...
		                                                m_sampling_ratio);
		if (procinfo.m_capacity_score < 0)
		{
			// Report negative capacity score as 0 and issue warning
			// this value was generated by get_system_capacity_score_bycpu_5()
			procinfo.m_capacity_score = 0;
			LOG_WARNING("Capacity score indicates server(s) are floating across CPUs.");
		}	
		proc->mutable_resource_counters()->set_capacity_score(
		    (uint32_t)(procinfo.m_capacity_score * 100));
		
		if (procinfo.m_stolen_capacity_score < 0)
		{
			// Report negative stolen capacity score as 0 and issue warning
			// this value was generated by get_system_capacity_score_bycpu_5()
			procinfo.m_stolen_capacity_score = 0;
			LOG_WARNING("Stolen capacity score indicates server(s) are floating across CPUs.");
		}
		proc->mutable_resource_counters()->set_stolen_capacity_score(
		    (uint32_t)(procinfo.m_stolen_capacity_score * 100));
		
		proc->mutable_resource_counters()->set_connection_queue_usage_pct(

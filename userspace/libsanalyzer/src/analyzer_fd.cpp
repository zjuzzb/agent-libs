...
sinsp_partial_transaction::type sinsp_proto_detector::detect_proto(
    sinsp_evt* evt,
    ...
    uint32_t buflen)
{
	...
	if (buflen >= MIN_VALID_PROTO_BUF_SIZE)
	{
		...
		// Detect HTTP
		//
		if (*(uint32_t*)buf == m_http_get_intval || *(uint32_t*)buf == m_http_post_intval ||
		    *(uint32_t*)buf == m_http_put_intval || *(uint32_t*)buf == m_http_delete_intval ||
		    *(uint32_t*)buf == m_http_trace_intval || *(uint32_t*)buf == m_http_connect_intval ||
                                         ...
                                         uint32_t len)
{
	...
	else if (fdinfo->is_ipv4_socket())
	{
		...
		{
			evt->m_tinfo->m_ainfo->m_th_analysis_flags |=
...
void sinsp_analyzer_fd_listener::update_transaction(sinsp_evt* evt,
                                                    int64_t fd,
                                                    sinsp_fdinfo_t* fdinfo,
                                                    char* data,
                                                    uint32_t original_len,
	                             ...
	                             trinfo->m_type <= sinsp_partial_transaction::TYPE_IP))
	{
		...
		if (trinfo->m_type <= sinsp_partial_transaction::TYPE_IP)
		{
			sinsp_partial_transaction::type type =
			    m_proto_detector.detect_proto(evt, trinfo, trdir, (uint8_t*)data, len);
...
void sinsp_analyzer_fd_listener::on_execve(sinsp_evt* evt)
{
	...
	if (m_analyzer->is_tracking_environment())
	{
		...
		{
			tinfo->m_ainfo->main_thread_ainfo()->hash_environment(
			    tinfo,
                                                      ...
                                                      const bool query_os_for_missing_info)
{
	...
	resolved = m_analyzer->resolve_custom_container(manager, tinfo, query_os_for_missing_info);
#endif
	return resolved;
}
...
void sinsp_analyzer_fd_listener::account_file_open(sinsp_threadinfo* tinfo,
                                                   const std::string& name,
                                                   uint32_t dev)
{
	if (!should_account_io(tinfo))
...
void sinsp_analyzer_fd_listener::account_error(sinsp_threadinfo* tinfo,
                                               const std::string& name,
                                               uint32_t dev)
{
	if (!should_account_io(tinfo))

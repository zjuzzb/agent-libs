	...
	*generic_match = true;
	for (const auto& cond : rule.m_cond)
	{
		...
		case object_filter_config::filter_condition::param_type::container_label:
		{
			...
			if (!lbl || fnmatch(cond.m_pattern.c_str(), lbl->c_str(), FNM_EXTMATCH))
			{
				matchcond = false;
				break;
			}
		...
		case object_filter_config::filter_condition::param_type::tag:
		{
			...
			if (!found || fnmatch(cond.m_pattern.c_str(), val.c_str(), FNM_EXTMATCH))
			{
				matchcond = false;
				break;
			}
...

{
	...
	{
		std::pair<bool, bool> matched = match_rule(rule,
		                                           rule_num,
		                                           tinfo,
		                                           mtinfo,
		                                           container,

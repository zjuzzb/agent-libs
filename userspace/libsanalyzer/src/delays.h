...
public:
	...
	void compute_program_delays(
	    std::vector<std::vector<sinsp_trlist_entry>>* host_client_transactions,
	...
	void compute_host_container_delays(
	    ...
	    std::vector<std::vector<sinsp_trlist_entry>>* server_transactions,
	    OUT sinsp_delays_info* delays);

	VISIBILITY_PRIVATE
	static uint64_t merge_transactions(std::vector<sinsp_trlist_entry>* intervals,
	                                   OUT std::vector<sinsp_trlist_entry>* merge,
	                                   bool do_sort);
	void compute_program_percpu_delays(
	    std::vector<std::vector<sinsp_trlist_entry>>* host_client_transactions,
	    std::vector<std::vector<sinsp_trlist_entry>>* host_server_transactions,
	    std::vector<std::vector<sinsp_trlist_entry>>* container_client_transactions,
	    std::vector<std::vector<sinsp_trlist_entry>>* container_server_transactions,
	...
	void compute_host_container_percpu_delays(
	    ...
	    int32_t cpuid,
	    sinsp_delays_info* delays);
	static uint64_t prune_client_transactions(
	    std::vector<std::vector<sinsp_trlist_entry>>* client_transactions_per_cpu,

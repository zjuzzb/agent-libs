...
void fill_common(const uid_set_t& parents, const container_group *congroup, k8s_common *common, const string& tag_prefix)
{
	...
	if(!tag_prefix.empty())
	{
		...
		for(const auto& tag : congroup->tags())
		{
			...
			{
				g_logger.format(sinsp_logger::SEV_NOTICE, "ignoring tag <%s> = <%s> for %s %s",
						tag_name.c_str(), tag_value.c_str(), kind.c_str(), id.c_str());
			}
		}
	}
...
template<> const unordered_map<string, setter_t<k8s_persistentvolumeclaim>> K8sResource<k8s_persistentvolumeclaim>::metrics(
	{
		{"kubernetes.persistentvolumeclaim.storage", SETTER(k8s_persistentvolumeclaim, set_storage)},
		{"kubernetes.persistentvolumeclaim.requests.storage", SETTER(k8s_persistentvolumeclaim, set_requests_storage)}
	});

template<> const string K8sResource<k8s_statefulset>::tag_prefix = "kubernetes.statefulset.";
template<> const unordered_map<string, setter_t<k8s_statefulset>> K8sResource<k8s_statefulset>::metrics(
	{
		...
		{"kubernetes.statefulset.status.replicas.current", SETTER(k8s_statefulset, set_status_replicas_current)},
		{"kubernetes.statefulset.status.replicas.ready", SETTER(k8s_statefulset, set_status_replicas_ready)},
		{"kubernetes.statefulset.status.replicas.updated", SETTER(k8s_statefulset, set_status_replicas_updated)},
	});

template<> const string K8sResource<k8s_replication_controller>::tag_prefix = "kubernetes.replicationController.";
template<> const unordered_map<string, setter_t<k8s_replication_controller>> K8sResource<k8s_replication_controller>::metrics(
	{
...
template<> const unordered_map<string, setter_t<k8s_hpa>> K8sResource<k8s_hpa>::metrics(
	{
...
template<> const string K8sResource<k8s_deployment>::tag_prefix = "kubernetes.deployment.";
template<> const unordered_map<string, setter_t<k8s_deployment>> K8sResource<k8s_deployment>::metrics(
	{
		...
		{"kubernetes.deployment.status.availableReplicas", SETTER(k8s_deployment, set_replicas_available)},
		{"kubernetes.deployment.status.unavailableReplicas", SETTER(k8s_deployment, set_replicas_unavailable)},
...
template<> const string K8sResource<k8s_daemonset>::tag_prefix = "kubernetes.daemonSet.";
template<> const unordered_map<string, setter_t<k8s_daemonset>> K8sResource<k8s_daemonset>::metrics(
	{
		{"kubernetes.daemonSet.status.currentNumberScheduled", SETTER(k8s_daemonset, set_current_scheduled)},
		{"kubernetes.daemonSet.status.desiredNumberScheduled", SETTER(k8s_daemonset, set_desired_scheduled)},
		{"kubernetes.daemonSet.status.numberMisscheduled", SETTER(k8s_daemonset, set_pods_misscheduled)},
		{"kubernetes.daemonSet.status.numberReady", SETTER(k8s_daemonset, set_pods_ready)},
	});

template<> const string K8sResource<k8s_job>::tag_prefix = "kubernetes.job.";
template<> const unordered_map<string, setter_t<k8s_job>> K8sResource<k8s_job>::metrics(
	{
...
template<> const string K8sResource<k8s_node>::tag_prefix = "kubernetes.node.";
template<> const unordered_map<string, setter_t<k8s_node>> K8sResource<k8s_node>::metrics(
	{
...
template<> const string K8sResource<k8s_replica_set>::tag_prefix = "kubernetes.replicaSet.";
template<> const unordered_map<string, setter_t<k8s_replica_set>> K8sResource<k8s_replica_set>::metrics(
	{
		...
		{"kubernetes.replicaset.status.fullyLabeledReplicas", SETTER(k8s_replica_set, set_replicas_fully_labeled)},
...
template<> const unordered_map<string, setter_t<k8s_resourcequota>> K8sResource<k8s_resourcequota>::metrics(
	{
		...
		{"kubernetes.resourcequota.requests.cpu.hard", [](k8s_resourcequota* pb, double value) -> void {
			pb->set_requests_cpu_hard(value);
			pb->set_double_request_cpu_hard(value);
		}},

		{"kubernetes.resourcequota.limits.cpu.used", [](k8s_resourcequota* pb, double value) -> void {
			pb->set_limits_cpu_used(value);
			pb->set_double_limits_cpu_used(value);
		}},
		{"kubernetes.resourcequota.requests.cpu.used", [](k8s_resourcequota* pb, double value) -> void {
			pb->set_requests_cpu_used(value);
			pb->set_double_request_cpu_used(value);
		}},

		{"kubernetes.resourcequota.limits.memory.hard", SETTER(k8s_resourcequota, set_limits_memory_hard)},
		{"kubernetes.resourcequota.requests.memory.hard", SETTER(k8s_resourcequota, set_requests_memory_hard)},
		{"kubernetes.resourcequota.limits.memory.used", SETTER(k8s_resourcequota, set_limits_memory_used)},
		{"kubernetes.resourcequota.requests.memory.used", SETTER(k8s_resourcequota, set_requests_memory_used)},

		//storage
		{"kubernetes.resourcequota.requests.storage.hard", SETTER(k8s_resourcequota, set_requests_storage_hard)},
		{"kubernetes.resourcequota.requests.storage.used", SETTER(k8s_resourcequota, set_requests_storage_used)},

		// count
		{"kubernetes.resourcequota.configmaps.hard", SETTER(k8s_resourcequota, set_configmaps_hard)},
		{"kubernetes.resourcequota.persistentvolumeclaims.hard", SETTER(k8s_resourcequota, set_persistentvolumeclaims_hard)},
		{"kubernetes.resourcequota.pods.hard", SETTER(k8s_resourcequota, set_pods_hard)},
		{"kubernetes.resourcequota.replicationcontrollers.hard", SETTER(k8s_resourcequota, set_replicationcontrollers_hard)},
		{"kubernetes.resourcequota.resourcequotas.hard", SETTER(k8s_resourcequota, set_resourcequotas_hard)},
		{"kubernetes.resourcequota.services.hard", SETTER(k8s_resourcequota, set_services_hard)},
		{"kubernetes.resourcequota.services.loadbalancers.hard", SETTER(k8s_resourcequota, set_services_loadbalancers_hard)},
		{"kubernetes.resourcequota.services.nodeports.hard", SETTER(k8s_resourcequota, set_services_nodeports_hard)},
		...
		{"kubernetes.resourcequota.persistentvolumeclaims.used", SETTER(k8s_resourcequota, set_persistentvolumeclaims_used)},
		{"kubernetes.resourcequota.pods.used", SETTER(k8s_resourcequota, set_pods_used)},
		{"kubernetes.resourcequota.replicationcontrollers.used", SETTER(k8s_resourcequota, set_replicationcontrollers_used)},
		{"kubernetes.resourcequota.resourcequotas.used", SETTER(k8s_resourcequota, set_resourcequotas_used)},
		{"kubernetes.resourcequota.services.used", SETTER(k8s_resourcequota, set_services_used)},
		{"kubernetes.resourcequota.services.loadbalancers.used", SETTER(k8s_resourcequota, set_services_loadbalancers_used)},

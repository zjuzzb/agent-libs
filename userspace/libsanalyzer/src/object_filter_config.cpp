...
filter_condition::param_type filter_condition::param2type(std::string pstr)
{
	static const map<std::string, param_type> param_map = {{"port", port},
...
std::string port_filter_rule::to_string() const
{
	std::stringstream out;
	out << (m_include ? "include " : "exclude ");
	if (m_use_set)
	{
		out << "set: ";
		for (uint32_t i : m_port_set)
			out << i << " ";
	}
	else
	{
		out << "range: " << m_range_start << "-" << m_range_end;
	}

	return out.str();
}

std::string filter_condition::to_string() const
{
	std::stringstream out;
	switch (m_param_type)
	{
...
std::string filter_rule::to_string() const
{
	std::stringstream out;
	out << (m_include ? "INCLUDE" : "EXCLUDE");
	for (uint32_t i = 0; i < m_cond.size(); i++)
	{
		out << "\n\tCondition " << i << ": ";
		out << m_cond[i].to_string();
	}
...
std::string object_filter_config_data::value_to_string() const
{
	...
	{
		out << "\nRule " << i << ": ";
		out << m_data[i].to_string();
	}

	return out.str();
}

std::string object_filter_config_data::value_to_yaml() const
{
    ...
    object_filter_config::port_filter_rule& rhs)
{
	...
	for (auto rule_it = node.begin(); rule_it != node.end(); rule_it++)
	{
                                                        ...
                                                        object_filter_config::rule_config& rhs)
{
	...
	for (auto conf_line = node.begin(); conf_line != node.end(); conf_line++)
	{
		if (!conf_line->first.IsScalar())
		{
			continue;
		}
		if ((conf_line->first.as<string>() == "port_filter") && conf_line->second.IsSequence())
		{
			rhs.m_port_rules =
			    get_sequence<object_filter_config::port_filter_rule>(conf_line->second);
		}
                                                        ...
                                                        object_filter_config::filter_rule& rhs)
{
	...
	for (auto rule_it = node.begin(); rule_it != node.end(); rule_it++)
	{
		...
		for (auto cond_it = rule_it->second.begin(); cond_it != rule_it->second.end(); cond_it++)
		{
			...
			if (cond.m_param_type == pf_flt_cond::param_type::container_label)
			{
				...
				cond.m_param =
				    cond.m_param.substr(object_filter_config::CONTAINER_LABEL.size() + 1);
			}

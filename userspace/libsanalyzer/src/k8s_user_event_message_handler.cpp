	  ...
	  m_connect_interval(default_connect_interval)
{
	m_callback = [this] (bool successful, google::protobuf::Message *response_msg) {
		...
		if(successful) {
			...
			glogf(sinsp_logger::SEV_DEBUG, "k8s_state: Got k8s event message.");
			handle_event(evt);
		} else {
			//
			// Error from cointerface, destroy the whole state and subscribe again
			//
			glogf(sinsp_logger::SEV_WARNING, "k8s_state: Error while receiving k8s event message. Reset and retry.");
			m_connected = false;
			reset_connection();
		}
	};
...
void k8s_user_event_message_handler::handle_event(sdc_internal::k8s_user_event *evt)
{
	...
	if ((ts != (time_t)-1) && (ts > (time_t)0))
	{
		...
		g_logger.log(logstr, (evt->type() == "Warning") ? sinsp_logger::SEV_EVT_WARNING : sinsp_logger::SEV_EVT_INFORMATION);
	} else {
		glogf(sinsp_logger::SEV_INFO,
...
void k8s_user_event_message_handler::subscribe(uint64_t timeout_s, user_event_filter_t::ptr_t filter)
{
	...
	glogf(sinsp_logger::SEV_INFO,
	      "k8s_state: Subscribe to k8s event messages, reconnect interval: %d sec",
	      timeout_s);
...
void k8s_user_event_message_handler::connect(uint64_t ts)
{
	if (m_connected)
	{
		glogf(sinsp_logger::SEV_DEBUG,
		      "k8s_state: Ignoring k8s message connection attempt because an RPC is already active");
		return;
	}
	m_connect_interval.run(
		...
		{
			glogf(sinsp_logger::SEV_INFO,
				"k8s_state: Connect to k8s event messages");

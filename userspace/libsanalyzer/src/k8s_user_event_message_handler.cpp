      ...
      m_infra_state(infra_state)
{
	m_callback = [this](bool successful, google::protobuf::Message* response_msg) {
		...
		{
			LOG_DEBUG("k8s_user_event: not delegated, skip event");
			return;
		}
		...
		if (successful)
		{
			...
			LOG_DEBUG("k8s_user_event: Got k8s event message.");
		...
		else
		{
			...
			//
			LOG_WARNING(
			    "k8s_user_event: Error while receiving k8s event message. Reset and retry.");
			m_connected = false;
			reset_connection();
		}
	};
...
void k8s_user_event_message_handler::handle_event(sdc_internal::k8s_user_event* evt,
                                                  infrastructure_state* infra_state)
{
	if (!m_event_filter)
	{
		LOG_DEBUG("k8s_user_event: no filter found");
		return;
	}
	...
	if (!allowed && evt->has_obj() && !evt->obj().kind().empty())
	{
		allowed = m_event_filter->allows_all(evt->obj().kind()) ||
		          (!evt->reason().empty() && m_event_filter->has(evt->obj().kind(), evt->reason()));
	}
	...
	if (!allowed)
	{
		LOG_DEBUG("k8s_user_event: no filter match for %s, %s",
		          evt->obj().kind().c_str(),
		          evt->reason().c_str());
	...
	LOG_DEBUG("k8s_user_event: filter match for %s, %s",
	          evt->obj().kind().c_str(),
	          evt->reason().c_str());
	...
	if ((ts != (time_t)-1) && (ts > (time_t)0))
	{
		if (evt->has_obj())
		{
			...
			if (!evt->obj().kind().empty() && !evt->obj().uid().empty() && infra_state)
			{
				...
				LOG_DEBUG("k8s_user_event: got %d scopes for %s:%s",
				          scope_names,
				          kind.c_str(),
				          evt->obj().uid().c_str());
			}
			if (scope_names < 1)
			{
				LOG_DEBUG("k8s_user_event: falling back to legacy scope for %s:%s",
				          evt->obj().kind().c_str(),
				          evt->obj().uid().c_str());
				...
				if (!evt->obj().kind().empty() && !evt->obj().name().empty())
				{
					...
					// I THINK this is because k8s_state parses the events this way (see
					// k8s_event.cpp and k8s_component.cpp). It is hideous.
		...
		auto severity = (evt->type() == "Warning") ? user_event_logger::SEV_EVT_WARNING
		                                           : user_event_logger::SEV_EVT_INFORMATION;
		auto evt = sinsp_user_event(ts,
		                            std::move(name),
		                            std::move(message),
		...
		LOG_DEBUG("k8s_user_event: new event update: %s", evt_str.c_str());

		// The specifial severities here are picked up by the glooger infrastructure and sent to the
		// right queue
		user_event_logger::log(evt, severity);
	}
	else
	{
		LOG_INFO("k8s_user_event: new event, bad timestamp %ld", evt->last_timestamp());
	}
}
...
void k8s_user_event_message_handler::subscribe(uint64_t timeout_s,
                                               user_event_filter_t::ptr_t filter)
{
	ASSERT(!m_connected);
	m_event_filter = filter;

	LOG_INFO("k8s_user_event: Subscribe to k8s event messages, reconnect interval: %ld sec",
	         timeout_s);
...
void k8s_user_event_message_handler::connect(uint64_t ts)
{
	...
	{
		LOG_DEBUG(
		    "k8s_user_event: Ignoring k8s message connection attempt because an RPC is already "
		    "active");
		return;
	}
	m_connect_interval.run(
	    [this]() {
		    LOG_INFO("k8s_user_event: Connect to k8s event messages");
		    ...
		    *cmd.mutable_include_types() = {
		        infrastructure_state::c_k8s_include_types.get_value().begin(),
		        infrastructure_state::c_k8s_include_types.get_value().end()};
...
void k8s_user_event_message_handler::refresh(uint64_t ts)
{
	...
	{
		ASSERT(m_subscribed);
		m_refresh_interval.run([this]() { m_coclient.process_queue(); }, ts);
	}
...
void k8s_user_event_message_handler::send_k8s_option(std::string key, std::string value)
{
	coclient::response_cb_t callback = [this](bool successful,
		...
		if (successful)
		{
			...
			LOG_DEBUG("k8s_user_event: Got option result message, success: %s, message: %s",
			          res->successful() ? "true" : "false",
			          res->has_errstr() ? res->errstr().c_str() : "");
		}
		else
		{
			LOG_WARNING("k8s_user_event: Error setting k8s option");
		}
	};

	...
	if (tr->m_protoparser->m_is_valid)
	{
		...
		if (is_server)
		{
			if (m_server_queries.size() < MAX_THREAD_REQUEST_TABLE_SIZE ||
			    m_server_queries.count(statement))
			{
				entry = &(m_server_queries[statement]);
				request_sorter<string, sinsp_query_details>::update(entry,
				                                                    tr,
				                                                    time_delta,
				                                                    is_error,
				                                                    m_percentiles);
			...
			if (!tables.empty())
			{
				...
				if (m_server_tables.size() < MAX_THREAD_REQUEST_TABLE_SIZE ||
				    m_server_tables.count(trunc_table))
				{
			...
			request_sorter<uint32_t, sinsp_query_details>::update(type_entry,
			                                                      tr,
			                                                      time_delta,
			                                                      is_error,
			                                                      m_percentiles);

			request_sorter<uint32_t, sinsp_request_details>::update(&m_server_totals,
			                                                        tr,
			                                                        time_delta,
			                                                        is_error,
			                                                        m_percentiles);
		...
		else
		{
			if (m_client_queries.size() < MAX_THREAD_REQUEST_TABLE_SIZE ||
			    m_client_queries.count(statement))
			{
			...
			{
				auto trunc_table = truncate_str(tables, truncation_size);
				if (m_client_tables.size() < MAX_THREAD_REQUEST_TABLE_SIZE ||
				    m_client_tables.count(trunc_table))
				{
			...
			request_sorter<uint32_t, sinsp_request_details>::update(&m_client_totals,
			                                                        tr,
			                                                        time_delta,
			                                                        is_error,
			                                                        m_percentiles);
...
inline void sql_state::add(sql_state* other)
{
	request_sorter<string, sinsp_query_details>::merge_maps(&m_server_queries,
	                                                        &(other->m_server_queries));
	request_sorter<string, sinsp_query_details>::merge_maps(&m_client_queries,
	                                                        &(other->m_client_queries));

	request_sorter<string, sinsp_query_details>::merge_maps(&m_server_tables,
	                                                        &(other->m_server_tables));
	request_sorter<string, sinsp_query_details>::merge_maps(&m_client_tables,
	                                                        &(other->m_client_tables));

	request_sorter<uint32_t, sinsp_query_details>::merge_maps(&m_server_query_types,
	                                                          &(other->m_server_query_types));
	request_sorter<uint32_t, sinsp_query_details>::merge_maps(&m_client_query_types,
	                                                          &(other->m_client_query_types));
...
void sinsp_protostate::to_protobuf(draiosproto::proto_info* protobuf_msg,
                                   uint32_t sampling_ratio,
                                   uint32_t limit)
{
	//
...
void sql_state::to_protobuf(draiosproto::sql_info* protobuf_msg,
                            uint32_t sampling_ratio,
                            uint32_t limit)
{
	if (m_server_queries.size() != 0)
	{
		...
		query_type_table_to_protobuf(protobuf_msg,
		                             &m_server_query_types,
		                             true,
		                             sampling_ratio,
		                             limit);
	}
	...
	{
		query_table_to_protobuf(protobuf_msg,
		                        &m_client_queries,
		                        false,
		                        sampling_ratio,
		                        true,
...
void sql_state::coalesce_protobuf(draiosproto::sql_info* protobuf_msg, uint32_t sampling_ratio)
{
...
{
	request_sorter<uint32_t, sinsp_query_details>::merge_maps(&m_server_ops,
	                                                          &(other->m_server_ops));
	request_sorter<uint32_t, sinsp_query_details>::merge_maps(&m_client_ops,
	                                                          &(other->m_client_ops));

	request_sorter<std::string, sinsp_query_details>::merge_maps(&m_server_collections,
	                                                             &(other->m_server_collections));
	request_sorter<std::string, sinsp_query_details>::merge_maps(&m_client_collections,
	                                                             &(other->m_client_collections));

	m_server_totals += other->m_server_totals;
	m_client_totals += other->m_client_totals;
}

inline void mongodb_state::update(sinsp_partial_transaction* tr,
                                  uint64_t time_delta,
                                  bool is_server,
                                  uint32_t truncation_size)
{
		...
		if (is_server)
		{
			if (pp->m_collection != NULL)
			{
				...
				if (m_server_collections.size() < MAX_THREAD_REQUEST_TABLE_SIZE ||
				    m_server_collections.count(collection))
				{
					collection_entry = &(m_server_collections[collection]);
					request_sorter<string, sinsp_query_details>::update(collection_entry,
					                                                    tr,
					                                                    time_delta,
					                                                    is_error,
					                                                    m_percentiles);
			...
			request_sorter<uint32_t, sinsp_query_details>::update(op_entry,
			                                                      tr,
			                                                      time_delta,
			                                                      is_error,
			                                                      m_percentiles);
		...
		else
		{
			if (pp->m_collection != NULL)
			{
				...
				if (m_client_collections.size() < MAX_THREAD_REQUEST_TABLE_SIZE ||
				    m_client_collections.count(collection))
				{
...
void mongodb_state::collections_to_protobuf(
    unordered_map<string, sinsp_query_details>& map,
...
void mongodb_state::to_protobuf(draiosproto::mongodb_info* protobuf_msg,
                                uint32_t sampling_ratio,
                                uint32_t limit)
{
	...
	collections_to_protobuf(
	    m_server_collections,
	    [protobuf_msg]() { return protobuf_msg->add_server_collections(); },
	    sampling_ratio,
	    limit);
	collections_to_protobuf(
	    m_client_collections,
	    [protobuf_msg]() { return protobuf_msg->add_client_collections(); },
	    sampling_ratio,
	    limit);

...
type_config<bool> secure_audit::c_secure_audit_file_writes_cmdline(
	false,
	"If true, secure_audit will enrich files opened for writing with command lines of the process opening them",
	"secure_audit_streams",
	"file_writes_cmdline");
...
void secure_audit::init(sinsp_ipv4_connection_manager* conn, sinsp_analyzer_fd_listener* analyzer_fd_listener)
{
	...
	if(c_secure_audit_file_writes_enabled.get_value())
	{
		...
		{
			wildcard_filter<std::string> filter(false, pattern,
		...
		// Register callback
		analyzer_fd_listener->subscribe_on_file_open_write([this](sinsp_threadinfo* tinfo,
									  uint64_t ts,
...
void secure_audit::emit_file_access_async(sinsp_threadinfo* tinfo, uint64_t ts, const std::string& fullpath, uint32_t flags)
{
	...
	{
		if(tinfo == nullptr || tinfo->m_ainfo == nullptr)
		{
			return;
		}

		if(!(tinfo->m_ainfo->m_th_analysis_flags & thread_analyzer_info::flags::AF_IS_INTERACTIVE_COMMAND))
		{
			m_file_accesses_not_interactive_dropped_count++;
			return;
		}
	...
	if(std::find(
		   secure_audit::BLACKLISTED_FILES.begin(),
		   secure_audit::BLACKLISTED_FILES.end(), fullpath) != secure_audit::BLACKLISTED_FILES.end())
	{
		return;
	}

...
bool should_drop(sinsp_evt* evt, bool* stopped, bool* switched)
{
	...
	for (j = 0; j < sizeof(g_events_to_absolutely_keep) / sizeof(g_events_to_absolutely_keep[0]);
	     j++)
	{
...
bool get_proc_mem_and_cpu(long& kb, int& cpu, std::string* err)
{
	...
	if (prev_tot_time)
	{
		...
		if (tot_diff)
		{
			cpu_pct = (int)round((((double)tot_cpu / tot_diff) * 100. /*%*/ * 100. /*scale*/) /
...
nsenter::nsenter(int pid, const std::string& type) : m_type(type)
{
...
void threshold_filter<double>::log(double value)
{
	g_logger.format(sinsp_logger::SEV_WARNING,
	                "%s above threshold curr=%.2f/%.2f %u:%u",
	                m_desc,
	                value,
	                m_threshold,
	                m_ntimes,
...
{
	g_logger.format(sinsp_logger::SEV_WARNING,
	                "%s above threshold curr=%ld/%ld %u:%u",
	                m_desc,
	                value,
	                m_threshold,
	                m_ntimes,
...
extern "C"
{
	...
	{
		static void (*const __throw_exc)(void*, void*, void (*)(void*)) __attribute__((noreturn)) =

...
void sinsp_sched_analyzer2::update(sinsp_threadinfo* tinfo, uint64_t ts, int16_t cpu, int64_t nexttid)
{
	...
	{
		if(tinfo->m_ainfo->m_dynstate->m_cpu_time_ns.size() != m_ncpus)
		{
			ASSERT(tinfo->m_ainfo->m_dynstate->m_cpu_time_ns.size() == 0);
			tinfo->m_ainfo->m_dynstate->m_cpu_time_ns.resize(m_ncpus);
		}

		tinfo->m_ainfo->m_dynstate->m_cpu_time_ns[cpu] += delta;

...
TEST(aggregator, k8s_pod)
{
	...
	auto init_container = i->mutable_pod_status()->add_init_containers();
	init_container->set_id("bar");
	init_container->set_status_ready(1);
	init_container->set_restart_count(2);
	init_container->set_requests_cpu_cores(3);
	init_container->set_limits_cpu_cores(4);
	init_container->set_requests_mem_bytes(5);
	init_container->set_limits_mem_bytes(6);
	...
	EXPECT_EQ(output.kubernetes().pods()[0].pod_status().containers()[0].aggr_status_ready().sum(),
	          1);
	EXPECT_EQ(output.kubernetes().pods()[0].pod_status().containers()[0].aggr_restart_count().sum(),
	          2);
	EXPECT_EQ(
	    output.kubernetes().pods()[0].pod_status().containers()[0].aggr_requests_cpu_cores().sum(),
	    3);
	EXPECT_EQ(
	    output.kubernetes().pods()[0].pod_status().containers()[0].aggr_limits_cpu_cores().sum(),
	    4);
	EXPECT_EQ(
	    output.kubernetes().pods()[0].pod_status().containers()[0].aggr_requests_mem_bytes().sum(),
	    5);
	EXPECT_EQ(
	    output.kubernetes().pods()[0].pod_status().containers()[0].aggr_limits_mem_bytes().sum(),
	    6);

	EXPECT_EQ(output.kubernetes().pods()[0].pod_status().init_containers()[0].id(), "bar");
	EXPECT_EQ(
	    output.kubernetes().pods()[0].pod_status().init_containers()[0].aggr_status_ready().sum(),
	    1);
	EXPECT_EQ(
	    output.kubernetes().pods()[0].pod_status().init_containers()[0].aggr_restart_count().sum(),
	    2);
	EXPECT_EQ(output.kubernetes()
	              .pods()[0]
	              .pod_status()
	              .init_containers()[0]
	              .aggr_requests_cpu_cores()
	              .sum(),
	          3);
	EXPECT_EQ(output.kubernetes()
	              .pods()[0]
	...
	init_container->set_status_ready(100);
	init_container->set_restart_count(100);
	init_container->set_requests_cpu_cores(100);
	init_container->set_limits_cpu_cores(100);
	init_container->set_requests_mem_bytes(100);
	init_container->set_limits_mem_bytes(100);
...
TEST(aggregator, k8s_persistentvolumeclaim)
{
	...
	EXPECT_EQ(output.kubernetes().persistentvolumeclaims()[0].access_modes()[0],
	          draiosproto::VOLUME_ACCESS_MODE_READ_WRITE_ONCE);

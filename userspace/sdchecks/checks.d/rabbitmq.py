...
class RabbitMQ(AgentCheck):
    ...
    def __init__(self, name, init_config, agentConfig, instances=None):
        ...
        self.logging_interval = {
            "%s_start_time" % EXCHANGE_TYPE: time.time(),
    ...
    def _get_current_node(self, base_url, auth, ssl_verify, instance_proxy):
        if not self.node_name:
            resp_data = self._get_data(urljoin(base_url, "overview"), auth=auth,
        ...
        if len(data) > max_detailed:
            ...
            if diff_time > instance.get('logging_interval', LOGGING_INTERVAL):
                ...
                msg = (
                    ...
                    "you are interested in by editing the dragent.yaml configuration file"
                ).format(object_type, len(data), max_detailed)
                self.log.info(msg)

...
class Couchbase(AgentCheck):
    ...
    TO_SECONDS = {'ns': 1e9, 'us': 1e6, '\xc2\xb5s': 1e6, 'ms': 1e3, 's': 1}
    ...
    def _create_metrics(self, data, instance, instance_state, server, tags=None):
        # Get storage metrics
        ...
        for task in data['tasks_all']:
            ...
            for item_name, val in list(task.items()):
        ...
        # *We have generated metrics for all task and sending events only for 'rebalance' tasks
    ...
    def check(self, instance):
        ...
        self._create_metrics(data, instance, instance_state, server, tags=list(set(tags)))
    ...
    def get_data(self, server, instance):
        ...
        try:
            ...
            # Add all tasks for sending metrics by tasks
            couchbase['tasks_all'] = tasks
        ...
        if err and code != 0:
            ...
            for metric_data in bucket_stats:
                if "ep_io" in metric_data and ":" in metric_data:
                    ...
                    self.gauge(full_metric_name, float(value.strip()), tags=metric_tags, device_name=bucket_name)

...
import requests
from six import iteritems, itervalues, PY3, text_type
from six.moves.urllib.parse import urljoin, urlparse

from checks import AgentCheck
from config import _is_affirmative as is_affirmative
...
def from_instance(instance):
    ...
    pshard_stats_master_node_only = is_affirmative(instance.get('pshard_stats_master_node_only', False))
...
class ESCheck(AgentCheck):
    ...
    def check(self, instance):
        ...
        health_url, stats_url, pshard_stats_url, pending_tasks_url, master_node_url = self._get_urls(version, config.cluster_stats)
        ...
        es_node = list(stats_data.get('nodes').values()) if stats_data.get('nodes') else None
        diff_time = 0
        ...
        if config.pshard_stats_master_node_only:
            master_node_url = self._join_url(config.url, master_node_url)
    ...
    def _get_es_version(self, config):
        ...
        self.service_metadata('version', str(version))
    ...
    def _get_urls(self, version, cluster_stats):
        ...
        return health_url, stats_url, pshard_stats_url, pending_tasks_url, master_node_url

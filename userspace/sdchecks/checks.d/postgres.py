...
"""
            ...
            try:
                if host == 'localhost' and password == '':
                    ...
                    connection = psycopg2.connect(
                        "user=%s dbname=%s application_name=%s" % (user, dbname, "sysdig-agent")
                    )
                elif port != '':
                    connection = psycopg2.connect(
                        host=host,
    ...
    def check(self, instance):
        ...
        if ssl not in SSL_MODES:
            ssl = 'require' if _is_affirmative(ssl) else 'disable'
        ...
        tag_replication_role = _is_affirmative(instance.get('tag_replication_role', False))

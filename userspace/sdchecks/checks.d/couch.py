...
class CouchDb(AgentCheck):
    ...
    RATE_METRICS = set([
        ...
        "couchdb.httpd.requests",           # available in CouchDB v1
        "couchdb.couchdb.httpd.requests",   # available in CouchDB v2
    ])
    ...
    def check(self, instance):
        ...
        if self.checker is None:
            name = instance.get('node_name', server)
            ...
            try:
                ...
                raise Exception("Unable to talk to the server: {}".format(e))
        ...
        for key, stats in iteritems(overall_stats):
            for metric, val in iteritems(stats):
                if val['current'] is not None:
                    ...
                    if metric_name in self.agent_check.RATE_METRICS:
...
class CouchDB2:
        ...
        for key, value in iteritems(data):
            if "type" in value:
                if value["type"] == "histogram":
                    ...
                    metric_name = "{0}.{1}".format(prefix, key)
                    if metric_name in self.agent_check.RATE_METRICS:
                        self.rate(metric_name, value["value"], tags=tags)
                    elif "httpd_request_methods" in prefix:
                        tags_copy = list(tags)
                        tags_copy.append("method:" + key)
                        self.rate(metric_name, value["value"], tags=tags_copy)
                    elif "httpd_status_codes" in prefix:
                        tags_copy = list(tags)
                        tags_copy.append("code:" + key)
                        self.rate(metric_name, value["value"], tags=tags_copy)
                    else:
                        self.gauge(metric_name, value["value"], tags=tags)
    ...
    def _get_instance_names(self, server, instance):
        name = instance.get('node_name', None)
        if name is None:

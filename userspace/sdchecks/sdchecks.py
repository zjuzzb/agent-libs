...
class Application:
    ...
    def __init__(self, install_prefix):
        ...
        self.blacklisted_pidnames_log_interval = timedelta(minutes=self.config._yaml_config.get_single("blacklist_log_interval", default_value=5))
        self.blacklisted_pidnames_log_time = datetime.now() + timedelta(seconds=15)
        self.blacklisted_pidnames_log_flag = True

        self.inqueue = None
        self.outqueue = None
        timeout_value = self.config._yaml_config.get_single("blacklist_flush_interval", default_value=30)
    ...
    def run_check(self, response_body, pidname, check, conf, trc):
        ...
        try:
            ...
            try:
                ...
                if self.config.check_conf_by_name(check["name"]).get("blacklisting_timeout", 1) != -1:
                    self.blacklisted_pidnames.add(pidname)
                return False, 0

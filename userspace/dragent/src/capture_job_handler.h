...
public:
	/**
	 * there are cases where we just want to dump and don't care
	 * about details and just want to dump stuff. So this is a convenience
	 * api for those situations
	 */
	void yolo_dump(const std::string& reason);

	// Change the chunk size used for event captures. This is only
	// used for testing.
	void set_dump_chunk_size(uint64_t size) { m_max_chunk_size = size; }

	// Get the last time at which this thread checked for job requests
	uint64_t get_last_job_check_ns() { return m_last_job_check_ns; }

	inline Poco::RWLock& jobs_lock() { return m_jobs_lock; }
	...
	bool get_job_in_progress() const { return m_job_in_progress; }
...
private:
	...
	void start_job(std::string& token, capture_job_queue_handler::start_job_details& request);
	...
	std::shared_ptr<serialized_buffer> dump_response_to_queue_item(
	    const draiosproto::dump_response& response);
	bool queue_item(std::shared_ptr<serialized_buffer>& item,

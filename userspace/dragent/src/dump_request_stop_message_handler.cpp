...
namespace dragent
{
dump_request_stop_message_handler::dump_request_stop_message_handler(dump_job_request_queue& queue)
    : m_job_request_queue(queue)
{
}

bool dump_request_stop_message_handler::handle_message(const draiosproto::message_type,
                                                       ...
                                                       const size_t buffer_size)
{
	...
	job_request->m_stop_details = make_unique<capture_job_queue_handler::stop_job_details>();

	job_request->m_request_type = capture_job_queue_handler::dump_job_request::JOB_STOP;
	...
	// need to add any state when stopping a job. We used to send this to sinsp worker
	// to help guarantee a stop wouldn't arrive before a start, however, the
	// connection_manager guarantees in order delivery.
	std::string errmsg;
	bool rc = m_job_request_queue.queue_job_request(job_request, errmsg);
	if (!rc)
	{
		LOG_ERROR("Could not queue dump stop request: %s", errmsg.c_str());
	}
	return rc;
}

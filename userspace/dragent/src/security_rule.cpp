                                             ...
                                             sinsp_container_manager& container_manager) const
{
	...
	// The following two blocks should probably be combined at some point, and use
	// infra-state exclusively for k8s related scoping, and container_manager exclusive
	// for container ones, but in order to start enable infra_state scoping without
	// risking current behavior, we keep them distinct for now.
	if (infra_state)
	{
		...
		// The asssumption here is that if we are attemption to check for only on host
		// or not on host, that that is the only valid non-agent-tag scope. This is enforced
		// at the backend, which does not allow custom scoping if "host" scoping us used. It
		// should probably be validated here. In any case,
		// we may have to translate it. This seems an optimization to avoid checking
		// each scope for "container.id" in cases when we will not have to perform that translation
		// (i.e. when there are more than one remaining predicate)
		if (remaining_predicates.size() == 1)
		{
			const draiosproto::scope_predicate& pred = remaining_predicates[0];

			if (pred.key() == "container.id" && pred.op() == draiosproto::EQ &&
			    pred.values().size() == 1 && pred.values()[0] == "")
			{
			...
			if (pred.key() == "container.id" && pred.op() == draiosproto::NOT_EQ &&
			    pred.values().size() == 1 && pred.values()[0] == "")
			{
				use_empty_preds = true;
				if (container_id == "")
		...
		return remaining_predicates.empty() ||
		       infra_state->match_scope(uid,
		                                (use_empty_preds ? empty_preds : remaining_predicates));
	}
	else
	{
		// This is a tough one. What do we do if there is a predicate which requires
		// information that we don't have about a container? We either run rules against
		// a container where they shouldn't necessariy apply, and have extra alerts, or
		// don't run rules against containers where they SHOULD apply, and miss alerts.
		//
		// False positives are far better than false negatives.
		bool match_unknown_predicates = true;

		// The fake-container "" represents "on the host" and therefore does not behave
		// like a real container. It therefore demands special handling. In this case,
		// we attempt to match container ID, then fail ANY other predicate which might exist
		// for containers. I don't know if this is the right answer long term, but
		// given the current mechanism, it's as good as any.
		if (container_id == "")
		{
			match_unknown_predicates = false;
		}

		bool scope_matches = true;
		std::shared_ptr<const sinsp_container_info> container_info =
		    container_manager.get_container(container_id);
		for (const auto& predicate : remaining_predicates)
		{
			bool valid_predicate = true;
			std::string local_container_field;
			if (predicate.key() == "container.id")
			{
				local_container_field = container_id;
			}
			else if (predicate.key() == "container.name" && container_info)
			{
				local_container_field = container_info->m_name;
			}
			else if (predicate.key() == "container.image" && container_info)
			{
				local_container_field = container_info->m_image;
			}
			else if (predicate.key() == "container.image.repo" && container_info)
			{
				local_container_field = container_info->m_imagerepo;
			}
			else if (predicate.key() == "container.image.tag" && container_info)
			{
				local_container_field = container_info->m_imagetag;
			}
			else if (predicate.key() == "container.image.id" && container_info)
			{
				local_container_field = container_info->m_imageid.substr(0,12);
			}
			else if (predicate.key() == "container.image.digest" && container_info)
			{
				local_container_field = container_info->m_imagedigest;
			}
			else
			{
			...
			if (valid_predicate)
			{
				scope_matches &= scope_resolver_iface::match_predicate(predicate, local_container_field);
				LOG_DEBUG("Matching %s to %s. current scope match status: %s",
						  predicate.key().c_str(),
				          container_id.c_str(),
				          scope_matches ? "still matches" : "match failed");
			}
			else
			{
				// We can get an invalid predicate in a couple of known scenarios:
				// 1) the container we're trying to match is the bogus "host" container with id ""
				// 2) it is actually an unknown predicate key
				// 3) for some reason, we can't find the container info associated with the ID
				//
				// We take the predetermined action, either failing the match, or ignoring the predicate
				scope_matches &= match_unknown_predicates;
				LOG_WARNING("Unverifiable predicate %s. Assuming %s.", predicate.key().c_str(), match_unknown_predicates ? "match" : "no match");
			}
...
const security_rule_library::rule::process_details*
security_rule_library::rule::get_process_details()
{
	return m_process_details.get();
}

const security_rule_library::rule::filesystem_details*
security_rule_library::rule::get_filesystem_details()
{
	return m_filesystem_details.get();
}

const security_rule_library::rule::syscall_details*
security_rule_library::rule::get_syscall_details()
{
	return m_syscall_details.get();
}

const security_rule_library::rule::container_details*
security_rule_library::rule::get_container_details()
{
	return m_container_details.get();
}

const security_rule_library::rule::network_details*
security_rule_library::rule::get_network_details()
{
	return m_network_details.get();
}

bool security_rule_library::rule::parse_matchlist(struct match_list& list, const Json::Value& val)
{
	if (val.isNull() || !val.isObject() || !val.isMember("items") || !val["items"].isArray() ||
	    !val.isMember("matchItems") || !val["matchItems"].isConvertibleTo(Json::booleanValue))
	{
		return false;
	}
...
bool security_rule_library::rule::parse_process_details(const Json::Value& val)
{
	...
	return parse_matchlist(m_process_details->m_processes, val["processes"]);
}

bool security_rule_library::rule::parse_filesystem_details(const Json::Value& val)
{
	if (!val.isMember("readWritePaths") && !val.isMember("readPaths"))
	{
		return false;
	}
	...
	{
		if (!parse_matchlist(m_filesystem_details->m_readwrite_paths, val["readWritePaths"]))
		{
			return false;
		}
	}
	...
	{
		if (!parse_matchlist(m_filesystem_details->m_readonly_paths, val["readPaths"]))
		{
			return false;
		}
	}
...
bool security_rule_library::rule::parse_syscall_details(const Json::Value& val)
{
	...
	return parse_matchlist(m_syscall_details->m_syscalls, val["syscalls"]);
}
...
bool security_rule_library::rule::parse_container_details(const Json::Value& val)
{
	...
	return parse_matchlist(m_container_details->m_containers, val["containers"]);
...
{
	if ((!val.isMember("tcpListenPorts") && !val.isMember("udpListenPorts")) ||
	    !val.isMember("allOutbound") || !val["allOutbound"].isConvertibleTo(Json::booleanValue) ||
	    !val.isMember("allInbound") || !val["allInbound"].isConvertibleTo(Json::booleanValue))
	{
	...
	{
		if (!parse_matchlist(m_network_details->m_tcp_listen_ports, val["tcpListenPorts"]))
		{
			return false;
		}
	}
	...
	{
		if (!parse_matchlist(m_network_details->m_udp_listen_ports, val["udpListenPorts"]))
		{
			return false;
		}
	}
...
bool security_rule_library::parse(const Json::Value& val, rule_sptr& rule)
{
	...
	if (!val.isObject() || !val.isMember("name") ||
	    !val["name"].isConvertibleTo(Json::stringValue) || !val.isMember("details") ||
	    !val["details"].isObject() || !val["details"].isMember("ruleType") ||
...
bool security_rule_library::parse(const std::string& rule_defs_str)
{
	...
	try
	{
		...
		if (rule_defs.isNull() || !rule_defs.isArray())
		{
			return false;
		}
		...
		for (uint32_t i = 0; i < rule_defs.size(); i++)
		{
			...
			{
				LOG_ERROR("Could not parse rule " + to_string(i) + " from rules json array");
				return false;
			}

			if (m_rules.find(rule->name()) != m_rules.end())
			{
				LOG_ERROR("Could not add rule " + rule->name() +
...
std::list<security_rules::match_result>* security_rules::add_result(
...
security_rules::security_rules() : m_num_loaded_rules(0)
{
...
void security_rules::set_match_details(draiosproto::event_detail& details,
                                       bool match_items,
                                       sinsp_evt* evt)
{
	if (match_items)
...
void falco_security_rules::add_policy(policy_v2_sptr policy)
{
	if (m_falco_engine && policy->enabled())
	{
		...
		{
			LOG_DEBUG("Number of falco rules for policy " + policy->name() + " : " +
			          std::to_string(m_falco_engine->num_rules_for_ruleset(ruleset)));
		}
	...
	for (auto& ruleset : m_rulesets)
	{
		try
		{
			unique_ptr<falco_engine::rule_result> res =
			    m_falco_engine->process_sinsp_event(evt, ruleset.first);
			...
			else
			{
				...
				// We don't need to worry about which rule matched in a given ruleset--falco handles
				// that internally.
				match_result result;
				...
				// The output is returned both as a human-readable string with values embedded, as
				// well as a list of field + value tuplies. The tuples are the union of the fields
				// seen in the output string and all exception fields.
				string output;
				m_formatters->tostring(evt, res->format, &output);
				result.m_detail.mutable_output_details()->set_output(output);
				(*result.m_detail.mutable_output_details()->mutable_output_fields())["falco.rule"] =
				    res->rule;
				...
				{
					(*result.m_detail.mutable_output_details()
					      ->mutable_output_fields())[rof.first] = rof.second;
				}
...
std::list<security_rules::match_result>* falco_security_rules::match_event(json_event* evt)
{
	...
	for (auto& ruleset : m_rulesets)
	{
		...
		{
			unique_ptr<falco_engine::rule_result> res =
			    m_falco_engine->process_k8s_audit_event(evt, ruleset.first);
			...
			else
			{
				...
				for (const auto& rt : resolved_tokens)
				{
					...
					{
						(*result.m_detail.mutable_output_details()
						      ->mutable_output_fields())[rt.first] = rt.second;
					}
				}
...
void matchlist_security_rules::match_info_set::add(
    const matchlist_security_rules::match_info& minfo)
{
	m_cur_matches.insert(std::make_pair(minfo.m_rule->name(), minfo.m_policy->id()));
}

bool matchlist_security_rules::match_info_set::contains(
    const matchlist_security_rules::match_info& minfo)
{
	return (m_cur_matches.find(std::make_pair(minfo.m_rule->name(), minfo.m_policy->id())) !=
	        m_cur_matches.end());
}
...
{
template<>
struct hash<matchlist_security_rules::match_info>
{
...
bool operator==(const matchlist_security_rules::match_info& a,
                const matchlist_security_rules::match_info& b)
{
	return (a.m_policy.get() == b.m_policy.get() && a.m_rule.get() == b.m_rule.get() &&
...
void matchlist_security_rules::add_policy(policy_v2_sptr policy)
{
	for (auto& rule_name : policy->rule_names())
	{
		...
		{
			LOG_DEBUG("matchlist_security_rules: loading rules from policy " +
			          to_string(policy->id()));
...
bool net_inbound_rules::add_rule(policy_v2_sptr policy, rule_sptr rule)
{
	LOG_DEBUG("net_inbound_rules: loading rule " + rule->name() + " from policy " +
	          to_string(policy->id()));
...
bool tcp_listenport_rules::add_rule(policy_v2_sptr policy, rule_sptr rule)
{
	LOG_DEBUG("tcp_listenport_rules: loading rule " + rule->name() + " from policy " +
	          to_string(policy->id()));
...
bool udp_listenport_rules::add_rule(policy_v2_sptr policy, rule_sptr rule)
{
	LOG_DEBUG("udp_listenport_rules: loading rule " + rule->name() + " from policy " +
	          to_string(policy->id()));
                         ...
                         std::shared_ptr<security_evt_metrics> metrics)
{
	...
	for (uint32_t j = 0; j < PPM_EVENT_MAX; j++)
	{
		if (PPME_IS_ENTER(j))
		{
			...
			if (it == m_evtnums.end() || it->first != m_etable[j].name)
			{
				it =
				    m_evtnums.emplace_hint(it,
				                           std::make_pair(m_etable[j].name, std::list<uint32_t>()));
			}
...
bool syscall_rules::add_rule(policy_v2_sptr policy, rule_sptr rule)
{
	bool added = false;

	LOG_DEBUG("syscall_rules: loading rule " + rule->name() + " from policy " +
	          to_string(policy->id()));
	...
	for (auto& evtstr : details->m_syscalls.m_items)
	{
		...
		if (it != m_evtnums.end())
		{
			for (auto evtnum : it->second)
			{
				...
				LOG_DEBUG("syscall_rules: Adding event " + to_string(evtnum) + "/" + evtstr +
				          " to rule " + rule->name() + " match " + to_string(minfo.m_match_items));
		...
		if (it2 != m_syscallnums.end())
		{
			if (sinsp::simple_consumer_consider_syscallid(it2->second))
			{
				...
				LOG_DEBUG("syscall_rules: Adding syscall " + to_string(it2->second) + "/" + evtstr +
				          " to rule " + rule->name() + " match " +
				          to_string(details->m_syscalls.m_match_items));
...
bool matchlist_map_security_rules::add_rule(policy_v2_sptr policy, rule_sptr rule)
{
	bool added = false;

	LOG_DEBUG("matchlist_map_security_rules (" + to_string(rules_type()) + "): loading rule " +
	          rule->as_string() + " from policy " + to_string(policy->id()));
	...
	for (auto& val : list->m_items)
	{
		...
		if (cur == NULL)
		{
			...
			{
				LOG_ERROR(
				    "matchlist_map_security_rules::add_rule: Could not find list we just added?");
				return added;
			}
		}
...
void container_rules::split_components(const filter_value_t& val, filter_components_t& components)
{
	...
	// ^(?:((?=[^:]{4,253})[a-zA-Z0-9-]{1,63}(?:[.][a-zA-Z0-9-]{1,63})*)(?::?([0-9]{1,5}))?/)? ->
	// host.name:port/
	// ((?:[a-z0-9._-])*(?:/[a-z0-9._-]*)*)(?::((?![.-])[a-zA-Z0-9_.-]{1,128}))? --------------->
	// name:tag
...
bool process_rules::add_rule(policy_v2_sptr policy, rule_sptr rule)
{
	bool added = false;

	LOG_DEBUG("process_rules: loading rule " + rule->name() + " from policy " +
	          to_string(policy->id()));

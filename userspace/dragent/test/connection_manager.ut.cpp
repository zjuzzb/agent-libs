...
TEST_F(connection_manager_fixture, unacked_metrics)
{
	const size_t MAX_QUEUE_LEN = 64;
	// Build some boilerplate stuff that's needed to build a CM object
	cm_config config = base_config;
...
TEST_F(connection_manager_fixture, connection_reset)
{
	const size_t MAX_QUEUE_LEN = 64;
	// Build some boilerplate stuff that's needed to build a CM object
	cm_config config = base_config;
	...
	for (uint32_t i = 0; cm.get_state() != cm_state_machine::state::STEADY_STATE && i < 5000; ++i)
	{
		usleep(1000);
	}
	...
	// Now have the collector reset the connection
	fc.reset_connection();

	// We should see a disconnect and reconnect
	for (uint32_t i = 0; fc.get_num_disconnects() == 0 && i < 10000; ++i)
	{
		usleep(1000);
	}
